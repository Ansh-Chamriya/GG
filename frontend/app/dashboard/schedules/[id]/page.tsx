...
export default function ScheduleDetailPage(props: {
  params: Promise<{ id: string }>;
}) {
  const params = use(props.params);
  const router = useRouter();
  const { selectedSchedule, isLoading, error, generateWorkOrder } =
    useSchedules(params.id);
        ...
        <div className="px-4 py-5 sm:p-6">
          <dl className="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2">
            <div className="sm:col-span-1">
              ...
              <dd className="mt-1 text-sm text-gray-900 dark:text-white">
                <ScheduleFrequencyTag
                  type={selectedSchedule.frequencyType}
                  value={selectedSchedule.frequencyValue}
                />
              </dd>
            </div>
            <div className="sm:col-span-1">
            ...
            <div className="sm:col-span-1">
              ...
              <dd className="mt-1 text-sm text-gray-900 dark:text-white">
                {selectedSchedule.lastPerformed
                  ? new Date(
                      selectedSchedule.lastPerformed
                    ).toLocaleDateString()
                  : "Never"}
